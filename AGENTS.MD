...
int LoadConfig(const char* exePath, Configuration* config) {
    ...
    if (settings) {
        ...
        config->settings.windowWidth = width ? width->valueint : 400;
        config->settings.windowHeight = height ? height->valueint : 300;
    }
...
LIBS = -lcomctl32 -ladvapi32 -lshell32 -Llib -lcjson
...
gcc -Os -s -mwindows -static-libgcc -Iinclude \
